...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(instancenewrange())
        {
            ...
            if(instance_mode == TAG)
            {
                sprintf((char*)&dataseq[0], "t %4.2f %4d %10d %04d %d %d", range_result, instance_get_lcount(), portGetTickCount(), instance_get_respPSC(), instance_get_txl(), instance_get_rxl());
                //sprintf((char*)&dataseq[20], " %08x", dwt_read32bitoffsetreg(0xa, 1));
            }
            else
            {
                sprintf((char*)&dataseq[0], "a %4.2f %4d %10d %04d %d %d", range_result, instance_get_lcount(), portGetTickCount(), (dwt_read16bitoffsetreg(0x10, 2) >> 4), instance_get_txl(), instance_get_rxl());
            }
