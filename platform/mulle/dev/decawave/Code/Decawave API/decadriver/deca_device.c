...
void dwt_isr(void) // assume interrupt can supply context
{
    ...
    if((status & SYS_STATUS_RXFCG) && (status & SYS_STATUS_LDEDONE))  // Receiver FCS Good
    {
        // read frame info
        uint16 len = 0;
		int overflow_check = dw1000local.dblbuffon;
		...
		while(overflow_check)
		{
			...
			{ 
				uint8 hsrb = 1;
				uint8 autorxen = (uint32)(dw1000local.sysCFGreg & SYS_CFG_RXAUTR) >> 24;
				//disble RX auto re-enable so the receiver does not enable before the overflow condition has been celared correctly
				if(autorxen) dwt_setautorxreenable(0);
				
				//clear all receive status bits (as we are finished with this receive event)
				dwt_write32bitreg(SYS_STATUS_ID,CLEAR_ALLRXGOOD_EVENTS) ;         // write status register to clear event bits we have seen
				//toggle buffer pointer twice to clear the overflow
				dwt_writetodevice(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET , 1, &hsrb) ;       

				dwt_write32bitreg(SYS_STATUS_ID,CLEAR_ALLRXGOOD_EVENTS) ;         // write status register to clear event bits we have seen
				dwt_writetodevice(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET , 1, &hsrb) ;       

				//enable auto rx re-enable
				if(autorxen)
				{ 
			...
			status = dwt_read32bitreg(SYS_STATUS_ID) ;            // check if overflow set

			overflow_check = status & SYS_STATUS_RXOVRR;
		}
		...
		if(overflow_check == 0)
		{
	        ...
	        {
	            dw1000local.cdata.aatset = (status & SYS_STATUS_AAT) ; //ACK request is set
	        }
	        ...
	        if(dw1000local.dblbuffon) //double RX buffer is enabled - toggle the host side pointer
	        {
				...
				if((dwt_read16bitoffsetreg(SYS_STATUS_ID, 0) & (uint16) SYS_STATUS_RXFCG) == 0)            // check if the SYS_STATUS_RXFCG event has cleared
				{
    ...
    {
		uint8 resetrx = 0xe0; //set RX reset bit... 
		...
		if(!(dw1000local.sysCFGreg & SYS_CFG_RXAUTR)) dwt_forcetrxoff(); //this will clear all events
		//set rx reset
		dwt_writetodevice(PMSC_ID, 0x3, 1, &resetrx);
